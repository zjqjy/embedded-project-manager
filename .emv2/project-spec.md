...
- **当前步骤**: S10 ⏳ S10-A/B/C/D/E 子代理开发完成（S10-E L1 通过 / L2 待用户）
- **整体状态**: S1-S6 全部完成 ✅ / S7 EM-SKILL GUI ⏸️ 暂停 / S8 模板优化 ✅ 完成 / S9 embed-ai-tool 整合 ✅ 完成 / S10 标准化+通用化+Git集成 ⏳ 子代理开发完成（L1 自动化 25/25 通过，L2 端到端验证待用户）
...
### 核心定位
**S10 = EM-SKILL 元升级 — 标准化(Skill-install 模板) + 通用化(双轨制) + Git 工作流集成**

EM-SKILL 走向通用工程类项目管理 skill，同时保留嵌入式场景作为"原生场景"。

### 子流程

| 子步骤 | 名称 | 状态 | 优先级 | 说明 |
|--------|------|------|--------|------|
| S10-A | SKILL.md 标准化 | ✅ 完成（子代理）| P0 | 顶部加 YAML frontmatter + 章节重排 + 工具索引 |
| S10-B | 目录通用化 | ✅ 完成（子代理）| P0 | `.emv2/` → `.em/` 双轨制（优先 `.em/`，回退 `.emv2/`）|
| S10-C | 嵌入式兼容 | ✅ 完成（子代理）| P0 | `em migrate` 命令 + 工具索引同步 + 文档补充 |
| S10-D | Git 工作流集成 | ✅ 完成（子代理）| P0 | initem 扩展 + CHANGELOG 脚本 + 提议 commit + tag |
| S10-E | 全流程验证 | ✅ 完成（子代理）| P0 | L1 自动化 25/25 通过 + 双场景 HVR 文件 + L2 待用户端到端验证 |

### 技术方案

| 项目 | 选择 |
|------|------|
| 标准化 | skill-install 模板（YAML frontmatter）|
| 加载模式 | 单 SKILL.md 整加载（不拆分扩展）|
| 目录命名 | `.em/` 主，`.emv2/` 兼容回退 |
| 嵌入式收纳 | `.em/embedded/{serial,flash,build,chip,integration}/` |
| init 智能识别 | Keil/CubeMX/ESP-IDF/PlatformIO 特征扫描 |
| Commit 规范 | EM 自定义 `[Sx] type: msg`（用户在 init 时可选）|
| CHANGELOG | EM 自写 Python 脚本（`tools/git-changelog/`）|
| Tag 策略 | 主步骤归档完成时打 `v0.10.0` 或 `em-s10-final` |
| AI 提交权限 | 提议 message + 文件列表，用户确认 commit；**禁止 push** |

### 嵌入式能力保留（关键原则）

- ✅ EM 起源于嵌入式，**串口/烧录/编译/芯片学习/embed-ai-tool 整合** 等核心能力完整保留
- ✅ 通用项目默认不加载 `.em/embedded/`，通过 init 智能识别或手动启用
- ✅ 现有 `.emv2/` 项目零配置升级
- ✅ 嵌入式"使用场景"作为 SKILL.md 章节
...
| S10 标准化+通用化+Git集成讨论 | `.emv2/discussion/20260602-skill-standardize-universal-git/` |
